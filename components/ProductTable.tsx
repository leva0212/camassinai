...
interface ProductTableProps {
  refresh?: boolean; // true para recargar desde fuera
}
...
export function ProductTable({ refresh }: ProductTableProps) {
  ...
  // Carga inicial y cuando refresh cambie
  useEffect(() => {
    cargar();
  }, [refresh]);
  ...
  const eliminar = async (id: number) => {
    ...
    await supabase.from("productos").delete().eq("prod_id", id);
        ...
        <tbody>
          {productos.map((p: any) => (
            <tr key={p.prod_id} style={{ borderTop: "1px solid #eee" }}>
              ...
              <td style={td}>{p.productos_categorias?.prod_cat_nombre || "-"}</td>
              <td style={td}>₡ {Number(p.prod_precio || 0).toLocaleString()}</td>
